...
void BackendHandler::GlfwWindowResizedCallback(GLFWwindow* window, int width, int height)
{
	...
	Application::Instance().ActiveScene->Registry().view<FilmGrain>().each([=](FilmGrain& buf)
		{
			buf.Reshape(width, height);
		});
	Application::Instance().ActiveScene->Registry().view<Pixelation>().each([=](Pixelation& buf)
		{
			buf.Reshape(width, height);
		});
