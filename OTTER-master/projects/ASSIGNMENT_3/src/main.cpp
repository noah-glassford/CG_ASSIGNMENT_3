...
#include <Graphics/Post/Pixelation.h>
#include <Graphics/Post/Bloom.h>
...
#pragma region Shader and ImGui
		...
		//Init gBuffer shader
		Shader::sptr qBufferShader = Shader::Create();
		qBufferShader->LoadShaderPartFromFile("shaders/vertex_shader.glsl", GL_VERTEX_SHADER);
		qBufferShader->LoadShaderPartFromFile("shaders/gBuffer_pass_frag.glsl", GL_FRAGMENT_SHADER);
		qBufferShader->Link();
		...
		BackendHandler::imGuiCallbacks.push_back([&]() {
			if (ImGui::CollapsingHeader("Effect controls"))
			{
				...
				if (activeEffect == 0)
				{
					...
					Pixelation* temp = (Pixelation*)effects[activeEffect];
					float intensity = temp->m_PixelCount;

					if (ImGui::SliderFloat("Intensity", &intensity, 1.0f, 800.0f))
					{
						temp->m_PixelCount = intensity;
					}

				}
				if (activeEffect == 1)
				{
					ImGui::Text("Active Effect: FilmGrain Effect");
					FilmGrain* temp = (FilmGrain*)effects[activeEffect];
					float intensity = temp->m_Intensity;

					if (ImGui::SliderFloat("Intensity", &intensity, 0.0f, 1.0f))
					{
						temp->m_Intensity = intensity;
					}
...
#pragma region Texture
		...
		Texture2D::sptr grass = Texture2D::LoadFromFile("images/floor.png");
...
// We need to tell our scene system what extra component types we want to support
		...
		simpleFloraMat->Shader = qBufferShader;
		...
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/air_enemy.obj");
			obj2.emplace<RendererComponent>().SetMesh(vao).SetMaterial(stoneMat);
			obj2.get<Transform>().SetLocalPosition(0.0f, 0.0f, 10.0f);
		...
		EnvironmentGenerator::AddObjectToGeneration("models/pillar_broken.obj", simpleFloraMat, 20,
			spawnFromHere, spawnToHere, allAvoidAreasFrom, allAvoidAreasTo);
		EnvironmentGenerator::AddObjectToGeneration("models/simpleRock.obj", simpleFloraMat, 12,
		...
		{
			PixelationEffect = &PixelationEffectObject.emplace<Pixelation>();
			PixelationEffect->Init(width, height);
		}
		effects.push_back(PixelationEffect);

		GameObject filmGrainEffectObject = scene->CreateEntity("FilmGrain Effect");
		{
